...
export default function StickyHeadTable(props) {
    ...
    const handleCancel = (pat_name,date,slot) => {
        setConfirmDialog({
            isOpen: true,
            title: 'Are you sure you want to cancel ?',
            subTitle: "You cannot undo this action",
            onConfirm: () => {onCancel(pat_name,date,slot)}
        });
    }

    const onCancel = async(pat_name,date,slot) => {
        setConfirmDialog({
            ...confirmDialog,
            isOpen: false
        });
        ...
        try {
            setError('');
            setLoading(false);
            await cancelAppointment(currentUser.email,pat_name,date,slot);
        } catch (error) {
                            ...
                            <TableCell align="start">
                                {
                                    ...
                                    ?   <Tooltip title="Cancel Appointment">
                                            <Button 
                                                color="secondary" 
                                                onClick={() => handleCancel(row.patientName,row.appointmentDate,row.time_slot)}
                                            >
