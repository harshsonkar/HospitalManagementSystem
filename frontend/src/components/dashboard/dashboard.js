...
import { AccountCircle, ChevronLeft, ExitToApp, Menu, Notifications } from '@mui/icons-material';
...
const DashboardContent = ({ title, children, mainListItems }) => {
  ...
  const [error, setError] = React.useState('') //setting error on top
  const { currentUser, logout } = useAuth();
  const history = useNavigate();
  
  const [confirmDialog, setConfirmDialog] = React.useState({isOpen: false, title: '', subTitle: ''});//logout conformation box
  const [openPopup, setOpenPopup]= React.useState(false); // notification popup
  const [notify, setNotify]= React.useState({isOpen: false, message: '', type:'info'}); // for notification popup

  const [open, setOpen] = React.useState(false); // dashboard popup
  const toggleDrawer = () => {
    setOpen(!open);
  };
...
const Dashboard=(props)=>{
  console.log(props);
  const {title, children, mainListItems} = props
  console.log(title, children, mainListItems)
  return(
    <DashboardContent 
      title={title} 
      children={children} 
